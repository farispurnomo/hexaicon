...
export default class PasswordStrength extends Plugin<PasswordStrengthOptions> {
    ...
    private validatePassword: () => ValidateFunction<ValidateOptions>;
    ...
    private checkPasswordStrength(): ValidateFunction<ValidateOptions> {
        return {
            validate: (input) => {
                ...
                const message =
                    result.feedback.warning || 'The password is weak';
