...
export default function identical(): ValidateFunctionInterface<
    IdenticalOptions,
    ValidateResult
> {
    return {
        validate(
            input: ValidateInput<IdenticalOptions, Localization>
        ): ValidateResult {
            const compareWith =
                'function' === typeof input.options.compare
                    ? (input.options.compare as CompareIdenticalCallback).call(
                          this
                      )
