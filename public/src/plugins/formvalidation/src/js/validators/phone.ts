...
export default function phone(): ValidateFunctionInterface<
    PhoneOptions,
    ValidateResult
> {
    ...
    return {
         ...
         */
        validate(
            input: ValidateInput<PhoneOptions, PhoneLocalization>
        ): ValidateResult {
            if (input.value === '') {
                return {
                    valid: true,
            ...
            switch (country.toUpperCase()) {
                ...
                case 'TH':
                    ...
                    isValid = /^0\(?([6|8-9]{2})*-([0-9]{3})*-([0-9]{4})$/.test(
                        v
                    );
                    break;
